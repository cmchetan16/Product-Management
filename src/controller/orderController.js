const { isValid, isValidObjectId, validBoolean, isValidRequestBody,isValidStatus, isValidNum} = require('../validation/validation.js')
...
//-----------------------------------------[Authoraisation checking]-----------------------------------------------//
        ...
        if(!validBoolean(cancellable)) return res.status(404).send({ status: false, message: "Please enter only true & false" })
...
const updateOrder = async function (req, res) {
    try { 
      ...
      if (!isValidStatus(status)) return res.status(400).send({ status: false, message: "status should be pending, completed,cancelled" })
        ...
        if(newOrder.cancellable==false && status== "cancelled") return res.status(400).send({ status: false, message: "you can not cancel this order" })
